...
CountryList::CountryList() //constructor
{
	head = nullptr;
}
...
bool CountryList::searchNode(string guess)
{
	...
	{
		if(nodeptr->CountryName==guess) //if same 
		{
...
void CountryList::countryGame(int& scores) //return scores
{
	string a_country[5] = { "england", "america","india","japan","china" };
	...
	string output = a_country[c_index];
	//word search
	if (a_country[c_index].length() % 2 == 0) {
		// for even string length case
		int row = rand() % 10;
		int col = rand() % 3;
		for (int i = 0; a_country[c_index][i] != '\0'; i++, col++) {
		    arr_country[row][col] = a_country[c_index][i];
		}
	}
	else {
	// for odd string length case
		int row = rand() % 3;
		int col = rand() % 10;
		for (int i = 0; a_country[c_index][i] != '\0'; i++, row++) {
		    arr_country[row][col] = a_country[c_index][i];
		}
	}
